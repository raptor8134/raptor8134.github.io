function ArticleBlock({block}){
  ...
  if(block.h)return <h2 style={{fontSize:"var(--size-2xl)",margin:"var(--space-7) 0 var(--space-3)",maxWidth:"32ch"}}>{block.h}</h2>;
  if(block.p)return <p style={{fontSize:"var(--size-md)",lineHeight:"var(--leading-body)",color:"var(--text-body)",maxWidth:"var(--measure)"}}>{block.lead&&<strong style={{color:"var(--text-strong)",fontWeight:600}}>{block.lead} </strong>}{block.p}</p>;
  if(block.list)return <div style={{margin:"var(--space-5) 0",maxWidth:"var(--measure)",borderTop:"var(--border-hairline)",borderBottom:"var(--border-hairline)",padding:"var(--space-4) 0"}}>
    {block.list.title&&<div style={{fontFamily:"var(--font-mono)",fontSize:"var(--size-2xs)",letterSpacing:"var(--track-caps)",textTransform:"uppercase",color:"var(--text-faint)",marginBottom:"var(--space-3)"}}>{block.list.title}</div>}
    <ul style={{margin:0,padding:0,listStyle:"none",display:"flex",flexDirection:"column",gap:"var(--space-2)"}}>
      {block.list.items.map((it,i)=><li key={i} style={{display:"flex",gap:"var(--space-3)",fontSize:"var(--size-sm)",lineHeight:1.5,color:"var(--text-body)"}}><span style={{fontFamily:"var(--font-mono)",fontSize:"var(--size-2xs)",color:"var(--accent-spring)",paddingTop:2}}>{String(i+1).padStart(2,"0")}</span><span>{it}</span></li>)}
    </ul></div>;
  if(block.skills)return <div style={{marginTop:"var(--space-7)",paddingTop:"var(--space-4)",borderTop:"var(--border-hairline)",fontFamily:"var(--font-mono)",fontSize:"var(--size-2xs)",lineHeight:1.8,color:"var(--text-faint)",maxWidth:"var(--measure)"}}>Skills: {block.skills}</div>;
  if(block.fig)return <div style={{margin:"var(--space-5) 0"}}><Figure {...block.fig}/></div>;
  if(block.table)return <div style={{margin:"var(--space-5) 0",maxWidth:"var(--measure)"}}><SpecTable {...block.table}/></div>;
  if(block.note)return <div style={{margin:"var(--space-5) 0",maxWidth:"var(--measure)"}}><Callout {...block.note}/></div>;
  return null;
}
...
function ProjectScreen({id,onBack,onOpen}){
  ...
  return (
    <article style={{maxWidth:820,margin:"0 auto",padding:"var(--space-7) var(--space-6) var(--space-9)"}}>
      ...
      <div style={{margin:"var(--space-5) 0 var(--space-6)",maxWidth:"var(--measure)"}}>
        <KeyValueList columns={2} items={p.meta}/>
      </div>
      <div>{(p.body||[{p:"Write-up in progress."}]).map((b,i)=><ArticleBlock key={i} block={b}/>)}</div>
